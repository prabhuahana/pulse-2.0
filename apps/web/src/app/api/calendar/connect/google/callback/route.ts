...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(
      `${getAppUrl()}/settings/calendar?error=${encodeURIComponent(error)}`
    );
  }

  if (!code || !(await verifyOAuthState("google", state))) {
    return NextResponse.redirect(
  ...
  try {
    ...
    await updateSessionCalendar((data) => ({ ...data, google: tokens }));
    return NextResponse.redirect(
      `${getAppUrl()}/settings/calendar?connected=google`
    );
  } catch (e) {
    const msg = e instanceof Error ? e.message : "google_connect_failed";
    return NextResponse.redirect(
