...
import { createThemeColor, darken, lighten, hexToRgba } from "@/lib/colors";
...
export function generateThemeFromColor(
  ...
  const colorObj = createThemeColor(primaryHex);
  if (!colorObj) return null;

  const accent = colorObj.hex;
  const accentLight = colorObj.shades.light;
  const accentDark = colorObj.shades.dark;

  if (isDark) {
    const bgColor = darken(accent, 60);
    const surfaceColor = darken(accent, 50);
    const textColor = lighten(accent, 70);
    const textMutedColor = lighten(accent, 40);

    return {
      id: "custom",
      label: "Custom",
      bg: bgColor,
      surface: hexToRgba(surfaceColor, 0.82),
      surfaceSolid: darken(accent, 45),
      text: textColor,
      ...
      gradient: `linear-gradient(135deg, ${bgColor} 0%, ${darken(
        accent,
        40
      )} 50%, ${accent} 100%)`,
    };
  } else {
    const bgColor = lighten(accent, 60);
    const surfaceColor = lighten(accent, 50);
    const textColor = darken(accent, 60);
    const textMutedColor = darken(accent, 30);

    return {
      id: "custom",
      label: "Custom",
      bg: bgColor,
      surface: hexToRgba(surfaceColor, 0.9),
      surfaceSolid: lighten(accent, 45),
      text: textColor,
      textMuted: textMutedColor,
      ...
      accentLight: accentLight,
      accentDark: accentDark,
      gradient: `linear-gradient(135deg, ${bgColor} 0%, ${lighten(
        accent,
        40
      )} 50%, ${lighten(accent, 55)} 100%)`,
    };
  }
}
